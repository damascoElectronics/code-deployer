...
class KeyPoolLogSimulator:
    ...
    def generate_log_entry(self):
        ...
        log_line = (
            ...
            f"createKey: KeyPoolService successfully created key "
            f"with identity = '{key_identity}', "
            f"sequence number {self.sequence_number}, and KeyPool "
            f"{{Source site identity = '{source_site}', "
            f"Destination site identity = '{self.site_id}', "
    ...
    def generate_metric_entry(self, key_count):
        ...
        key_log = (
            ...
            f"[quartzScheduler_Worker-{random.randint(1,10)}] "
            f"c.e.q.k.k.KeySyncServiceImpl             : "
            f"METRIC_RECEIVED_PUBLIC_KEY_COUNT BITS={bits} "
            f"KEYS={key_count}"
        )
...
def write_logs_to_file(log_lines, filename):
    ...
    logger.info("Written %d lines to %s", len(log_lines), filepath)
    return filepath
...
def generate_log_batch(simulator):
    ...
    log_filename = (
        f"keypool_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    )
