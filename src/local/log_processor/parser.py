...
class LogParser:
    ...
    def parse_log_entry(self, line):
        ...
        Returns:
            dict: Parsed entry with log type and extracted fields, or None if parsing fails
        """
        try:
            parts = line.split()
            if len(parts) < 5:
            ...
            if 'createKey' in line and 'sequence number' in line:
                ...
                identity_match = re.search(r"identity = '([a-f0-9-]{36})'", line)
                if identity_match:
                ...
                # Extract source site
                source_match = re.search(r"Source site identity = '(\d+)'", line)
                if source_match:
                    entry['source_site'] = int(source_match.group(1))
                
                # Extract destination site
                dest_match = re.search(r"Destination site identity = '(\d+)'", line)
                if dest_match:
                    entry['dest_site'] = int(dest_match.group(1))
                
                # Extract key pool type
                type_match = re.search(r"KeyPoolType name = '(PUBLIC|PRIVATE|SHARED)'", line)
                if type_match:
    ...
    def process_log_file(self, filepath):
        ...
        Returns:
            dict: Statistics about processed file, or None if processing fails
        """
        try:
            ...
            with open(filepath, 'r') as f:
                for line in f:
                    ...
                    if success:
                        ...
                        if entry.get('log_type') != 'UNKNOWN':
                            stats['db_errors'] += 1
            
            # Mark file as processed in database
            self.db_manager.mark_file_processed(filename, file_size, stats)
    ...
    def _store_entry(self, entry, filename):
        ...
        if log_type == 'KEY_CREATION':
            if all(k in entry for k in ['key_identity', 'sequence_number', 
    ...
    def _log_statistics(self, filename, stats):
        ...
        if stats['db_errors'] > 0:
            logger.warning(f"  DB errors: {stats['db_errors']}")