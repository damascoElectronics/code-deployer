...
class LogDownloader:
    ...
    def _load_processed_files(self):
        ...
        if os.path.exists(self.processed_files_log):
            with open(
                self.processed_files_log, 'r', encoding='utf-8'
            ) as file:
                return set(
    ...
    def mark_as_processed(self, filename):
        ...
        with open(
            self.processed_files_log, 'a', encoding='utf-8'
        ) as file:
    ...
    def check_api_health(self):
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(
                    "✓ Connected to VM2 API at %s", self.api_url
                )
                return True

            logger.warning(
                "VM2 API returned status %s", response.status_code
            )
            return False
    ...
    def get_new_logs(self):
        ...
        if new_logs:
            logger.info(
                "Found %s new log files to process", len(new_logs)
            )
        else:
            logger.info(
                "All %s logs already processed", len(available_logs)
            )
    ...
    def download_log_file(self, filename):
        ...
        try:
            ...
            logger.info(
                "✓ Downloaded %s (%s bytes)", filename, file_size
            )
